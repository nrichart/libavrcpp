...
/* -------------------------------------------------------------------------- */
template<typename _reg_t> struct reg_high          { enum { offset = 1 }; };
template<>                struct reg_high<uint8_t> { enum { offset = 0 }; };

/* -------------------------------------------------------------------------- */
template <typename reg, typename _reg_t>
struct regReaderHelper {
  static inline _reg_t val() { return reg::val_low(); }
};

template <typename reg>
struct regReaderHelper<reg, uint16_t> {
...
public:
  ...
  static inline volatile _reg_t * ptr() {
    return ((volatile _reg_t*) address);
  }
  ...
  static inline bool is_clear(uint8_t bit)
  { return  getbit(bit) == 0; }
  static inline bool is_set(uint8_t bit)
  { return  getbit(bit) == 1; }
