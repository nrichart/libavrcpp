...
/**
   ...
   Modifications:
   - 08/2014 This code has been modified and cleaned for the need of libavrc++
   by netWorms
*/
...
template <typename rst, typename a0, typename led, typename select = ss,
          typename spi_master = SPI0<select>>
class ST7565 {
public:
  enum { width = 128 };
  enum { height = 64 };
  ...
  static void activate() {
    ...
    spi_master::activate();
    spi_master::setMode(spi::_mode_0);
    spi_master::setBitOrder(spi::_bo_msb);
  ...
  template <class Color> static void setBacklightColor(const Color &color) {
    led::setColor(color);
  }
...
private:
  ...
  enum booster_t { _booster_234 = 0, _booster_5 = 1, _booster_6 = 3 };
...
template <typename rst, typename a0, typename led, typename select,
          typename spi_master>
uint8_t ST7565<rst, a0, led, select, spi_master>::page_map[8] = {3, 2, 1, 0,
                                                                 7, 6, 5, 4};
