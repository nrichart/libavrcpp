...
/**
   Modifications
   - 08/2014 This files has been modified for the need of the libavrc++ by netWorms
 */
...
struct gLCDBoundingBox {
  ...
  void update(uint8_t x, uint8_t y) {
    ...
    x_max = min(max(x, x_max), hw_lcd::width - 1);
    y_max = min(max(y, y_max), hw_lcd::height - 1);
  }

  void update(uint8_t x1, uint8_t y1,
	      uint8_t x2, uint8_t y2) {
    x_min = max(min(x1, x_min), 0);
    y_min = max(min(y1, y_min), 0);

    x_max = min(max(x2, x_max), hw_lcd::width - 1);
    y_max = min(max(y2, y_max), hw_lcd::height - 1);
  }

  uint8_t x_min, x_max, y_min, y_max;
};

template<class hw_lcd>
struct gLCDBoundingBox<hw_lcd, false> {
  gLCDBoundingBox() { clear(); }
...
#define MAX_STR_SIZE 64
  ...
  /* ------------------------------------------------------------------------ */
  static void drawBitmap_PF(uint8_t x, uint8_t y,
			    const uint8_t * bitmap, uint8_t w, uint8_t h,
			    bool flip = false) {
    for (uint8_t j = 0; j < h; ++j) {
      for (uint8_t i = 0; i < w; ++i) {
	uint8_t mask;
	if(flip) mask = _BV(7-j%8);
	else mask = _BV(j%8);
  ...
  /* ------------------------------------------------------------------------ */
  static void drawRectangle(uint8_t x1, uint8_t y1,
			    uint8_t x2, uint8_t y2,
			    uint8_t color = 0x1,
			    bool filled = false) {
...
private:
  static void setPixelWhithoutBBUpdate(uint8_t x, uint8_t y,
    ...
    if (color)
      buffer[x + (y/(hw_lcd::height/8))*hw_lcd::width] |= _BV(7-(y%8));
    else
      buffer[x + (y/(hw_lcd::height/8))*hw_lcd::width] &= ~_BV(7-(y%8));
  }
...
template<typename hw_lcd>
gLCDBoundingBox<hw_lcd> gLCD<hw_lcd>::bbox;
